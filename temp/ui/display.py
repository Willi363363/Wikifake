...
def print_game_content(game_data: dict):
    ...
    console.print("[bold]Ouvrez la page HTML qui vient de se lancer ![/bold]")
    console.print()
    
    # Afficher les instructions
    console.print(Panel(
        f"[bold yellow]🔎  Mode Fausse Discrétion activé ![/bold yellow]\n"
        f"[dim]Cet article contient {game_data['total_false_statements']} fausse(s) affirmation(s) secrètement ajoutée(s).[/dim]",
        border_style="yellow"
    ))
    console.print()


def print_solutions(game_data: dict):
    """Affiche directement les solutions dans le terminal"""
    console.print("\n[bold red]🚨 VOICI LES FAUSSES INFORMATIONS INJECTÉES DANS L'ARTICLE :[/bold red]")
    
    for info in game_data.get("positions", []):
        console.print(f"[bold white]Paragraphe {info['paragraph_index']}:[/bold white] [red]{info['false_statement']}[/red]")
        
    console.print("\n[dim]Regardez l'article dans votre navigateur : le mensonge est écrit comme si c'était vrai ![/dim]")
