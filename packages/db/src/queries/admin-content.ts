 ...
 */
export async function countCacheHits(db: Db): Promise<CacheCounts> {
  const [row] = await db
    .select({
 ...
 */
export async function countGenerationFailures(db: Db): Promise<GenerationFailures> {
  const [row] = await db
    .select({
...
export async function countDistinctTopics(db: Db): Promise<number> {
  const [row] = await db
...
export async function countGenerated(db: Db): Promise<number> {
  const [row] = await db
