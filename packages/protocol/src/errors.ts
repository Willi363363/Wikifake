...
export const ERROR_CODES = [
  ...
  'cosmetic_not_found',
  /**
   * N.5 — this account has already played the article of the day.
   *
   * **One attempt is what makes the board mean anything.** A player free to
   * replay until the score is good is ranked against their own patience rather
   * than against the others, and the top of the list becomes whoever retried
   * most.
   *
   * It says *already played* rather than *not allowed*, because the two read
   * very differently to somebody who has just opened the page and forgotten they
   * played at breakfast.
   */
  'daily_already_played',
  /**
   * N.5 — there is no article for today yet.
   *
   * **Not a failure of the request**, which is why it is its own code rather
   * than `generation_failed`: the day is either being generated by whoever asked
   * first, or a generation failed and the next request will try again. Both mean
   * *come back in a moment* to a player, and neither means they did anything
   * wrong.
   */
  'daily_not_ready',
] as const;
