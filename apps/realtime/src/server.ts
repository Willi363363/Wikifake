...
import { createAcceptor } from './accept.js';
...
import { CLOSE_POLICY_VIOLATION } from './frames.js';
import type { RoomStore } from './rooms/store.js';
import { createSubscriptions } from './subscriptions.js';
import type { RoundOutcome } from './generation.js';
import { DEFAULT_INTERVALS, type Intervals } from './throttle.js';
...
export function createService(options: ServiceOptions): Service {
  ...
  // Step Q.5 — one socket, from the upgrade to the join, in `accept.ts`.
  // Everything it needs is passed rather than closed over, which is what let
  // it move out of a file that had no room left for the `try` it needed.
  const accept = createAcceptor({
    options,
    connections,
    subscriptions,
    scheduler,
