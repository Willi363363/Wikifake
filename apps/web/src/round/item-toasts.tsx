...
export function ItemToasts({ landed, lastScan, onDismiss }: ItemToastsProps) {
  ...
  return (
    ...
    >
      {lastScan === null ? null : (
        <p className="rounded-lg bg-bronze-soft px-4 py-3 text-sm text-ink">
      ...
      {landed.map((each) => {
        ...
        return (
          <div
            key={each.id}
            className="flex items-center gap-3 rounded-xl bg-surface px-4 py-3 text-sm text-ink shadow-lg"
          >
