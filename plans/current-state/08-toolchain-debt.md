...
still to write — the merge button choosing squash for the fifth time running.
Everything about getting a batch from `staging` to `main` is now
`11-promotion-debt.md`: the promotion blocked by an already-merged commit, its
last step being a push the guard refuses, and the merge-method drift that keeps
reopening both.

What stayed here is what this register is for — the commands you run, and what
they do not tell you. The migration entry above is the boundary: *nothing in the
deploy path runs one* is a fact about the commands, and *which ones a promotion
must apply first* is a fact about the promotion.
